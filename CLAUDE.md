...
CI via GitHub Actions (`.github/workflows/ci.yml`): lint, format check, type check, and `pytest` on **every branch push** (`branches: ["**"]`), on PRs to `main`, and on demand (`workflow_dispatch`). Uses `macos-14` (Apple Silicon) runners for MLX compatibility. All four steps gate the build. Tags are excluded from the push trigger — `release.yml` owns `v*`; push the branch **before** tagging, since a tag ref never matches `push.branches` and `release.yml` has no status gate, so tagging an unpushed commit publishes a Release from code CI never ran. A `concurrency` group supersedes in-flight runs per branch ref; `main` runs are keyed on `github.run_id` and so sit outside the group entirely. **`cancel-in-progress: false` alone would not have been enough** — GitHub cancels an already-*pending* run in a group unconditionally when a new one queues, so a shared `main` group would let a third push discard the second's queued run and let one slow run head-of-line-block every later one. The repo is public, so Actions is free on these standard runners.
